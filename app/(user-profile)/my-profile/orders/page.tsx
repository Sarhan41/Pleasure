    ...
    include: {
      orderItems: {
        include: {
          ...
          color: true, // Ensure colors are included
        },
      },
    },
  });
      ...
      {orders.length === 0 ? (
        ...
        orders.map((order) => (
          <div key={order.id} className="bg-white shadow-lg rounded-lg p-6 mb-6">
            ...
            <p className="text-xl font-semibold mb-4">Paid: {order.isPaid ? "Yes" : "No"}</p>
                  ...
                  <Link href={`/product/${productName}`} key={item.id}>
                    <li className="py-4 w-fit flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        {item.color && (
                          <div>
                            <p className="text-sm text-gray-500">Colors: {item.color.map((color) => color.name).join(", ")}</p>
